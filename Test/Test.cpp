...
struct HugeStruct {
  ...
  HugeStruct(const HugeStruct& _rhs) : d(_rhs.d), f(_rhs.f), i(_rhs.i), c(_rhs.c) {
    memcpy(darr, _rhs.darr, sizeof(double) * 16);
  }
...
bool RandomDequeTest(size_t _count, uint32_t _seed, bool _is_verbose) {
  enum class Action { push_back = 0, push_front, pop_back, pop_front };
  ...
  for (unsigned i = 0; i < deque.size(); ++i) {
    if (deque[i] != expected[(start + i) % count]) {
      is_fail = true;
      output << "Value difference in index " << i << " with value " << deque[i] << " : (expected)"
             << expected[(start + i) % count] << std::endl;
    }
  }
  ...
  // Range based for test
  {
    ...
    for (auto& i : deque) {
      ...
      if (i != expected[(start + j) % count]) {
        is_fail = true;
        output << "Value difference during range based for loop " << " with value " << i << " : (expected)"
               << expected[(start + j) % count] << std::endl;
      }
...
bool RandomListTest(size_t _count, uint32_t _seed, bool _is_verbose) {
  ...
  for (size_t i = 0; i < expected_c; ++i) {
    if (lamp.front() != expected[(start + i) % _count]) {
      is_fail = true;
      output << "Value difference in index " << i << " with value " << lamp.front() << " : (expected)"
             << expected[(start + i) % _count] << std::endl;
    }
...
int main(int argc, const char* argv[]) {
  ...
  if (false) {
    ...
    }
    {
      ...
      std::cout << "[ " << result.c0.x << ", " << result.c1.x << ", " << result.c2.x << ", " << result.c3.x << "]"
                << std::endl;
      std::cout << "[ " << result.c0.y << ", " << result.c1.y << ", " << result.c2.y << ", " << result.c3.y << "]"
                << std::endl;
      std::cout << "[ " << result.c0.z << ", " << result.c1.z << ", " << result.c2.z << ", " << result.c3.z << "]"
                << std::endl;
      std::cout << "[ " << result.c0.w << ", " << result.c1.w << ", " << result.c2.w << ", " << result.c3.w << "]"
                << std::endl;
    }
  ...
  if (false) {
    ...
    for (size_t i = 0; i < v.size(); ++i) {
      ...
      if ((left < v.size() && v[i] < v[left]) || (right < v.size() && v[i] < v[right])) {
        std::cout << "Failed max heap test in v[" << i << "] = " << v[i] << ", left,right : " << v[left] << ' '
                  << v[right] << std::endl;
        break;
      }
      else if (v[i] > v[0]) {
    ...
    for (size_t i = 0; i < v.size(); ++i) {
      ...
      if ((left < v.size() && v[i] > v[left]) || (right < v.size() && v[i] > v[right])) {
        std::cout << "Failed min heap test in v[" << i << "] = " << v[i] << ", left,right : " << v[left] << ' '
                  << v[right] << std::endl;
        break;
      }
      else if (v[i] < v[0]) {
